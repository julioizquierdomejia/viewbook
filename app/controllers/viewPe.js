...
app.controller('viewPeController', ['$scope','globalSettingsService','utilService', '$timeout', 'activityService','$compile','sessionService', 'peConfigService', 'learningService', 'peDataService', function($s, gss, us, timeout, acts, compile, ses, pcs, lns, pds) {
  ...
  $s.noLoad = false;
  $s.class_code = 'base';
  $s.averageData = {'class':'navbar-light bg-light','class_color':'text-dark','icon':'fa-clock', 'text':''};
  $s.questions_head = [];
  $s.questions_detail = [];  
  $s.evaluation_log = []; 
  $s.scores_letters = [];
  ...
  $s.dataActivityTemp = { 'page': 0, 'key': 0 };
      ...
      if( type == "textarea" ){ 
        ...
        for(i=0; i<radioElement.length;i++){   
          ...
          if(radioElement[i].checked){      
            values.push({label: label.innerHTML, value: (radioElement[i].value == 'on') ? 'opcin-' + i : radioElement[i].value, 'selected' : true, 'index' : i+1 }) 
            value.value = label.innerHTML; 
            if(value.value === undefined || value.value == ""){
              vacio = true;
            }
          }else{
            values.push({label: label.innerHTML, value: (radioElement[i].value == 'on') ? 'opcin-' + i : radioElement[i].value, 'selected' : false, 'index' : i+1 })
          }    
        }   
        value.values = values;
        $s.dataFormFinal.push(value);
        ...
        for(i=0; i<radioElement.length;i++){   
          ...
          if(radioElement[i].checked){      
            values.push({label: label.innerHTML, value: (valueTemp.value == 'on') ? 'opcin-' + i : valueTemp.value, 'selected' : true, 'index' : i+1 })  
            value.value = label.innerHTML; 
            if(value.value === undefined || value.value == ""){
              vacio = true;
            }
          }else{
            values.push({label: label.innerHTML, value: (valueTemp.value == 'on') ? 'opcin-' + i : valueTemp.value, 'selected' : false, 'index' : i+1 })
          }
        }   
        value.values = values;
        $s.dataFormFinal.push(value);
  ...
  $s.generateJsonDragg = function(name){
    ...
    var jsondragg_left = [], jsondragg_right = [], jsondragg = [];
    var contitem = 0;
    [].forEach.call(theleft, function(item) {
      ...
      jsondragg_left.push( { 'item': contitem, 'nitem': nitem, 'text' : texto, 'img' : img } );
      contitem++;
    });
    contitem = 0; 
    [].forEach.call(theright, function(item) {  
      ...
      var img = ( nameimg != '' || nameimg !== undefined ) ? gss.path_pecontent_upload_activity + nameimg : false;
      jsondragg_right.push( { 'item': contitem, 'nitem': nitem, 'text' : texto, 'img' : img } );
      contitem++;
    });

    jsondragg.push({left: jsondragg_left, right: jsondragg_right});
    return jsondragg;
  }
  ...
  $s.sendTaskJoin = function(){
    $("div.mce-tinymce-inline").css({'display':'none !important'}); 
    $s.activityProccess = 3;
    $s.showLoadSAQ();
